...
| 6. フェーズ構成とマスタースケジュール | wbs.yaml, **proposal_draft.md** | フェーズ一覧、マスタースケジュール（提案書の導入ステップと整合） |
...
**wbs.yaml から:**
```yaml
# フェーズ構成の生成（セクション6とセクション7で共通）
フェーズリスト = tasks.phase の一意な値をリスト化（出現順）
  ※ セクション6「フェーズ構成とマスタースケジュール」
  ※ セクション7「フェーズ別タスク・成果物」
  → 両セクションで同じフェーズリストを使用すること

# セクション6: マスタースケジュール
各フェーズ:
  - フェーズ名、目的、期間、主要成果物の概要

# セクション7: フェーズ別詳細
各フェーズ:
  - タスク一覧 = tasks[phase == target_phase]
  - 成果物一覧、担当者、依存関係

# スケジュール生成
開始日 = min(tasks.due) または tasks[0].due - 見積期間
終了日 = max(tasks.due)
各フェーズの期間 = tasks[phase == X]のdue範囲
```
...
| **フェーズ一貫性** | セクション6と7のフェーズリストが完全に一致しているか | セクション6に4フェーズ、セクション7に3フェーズ |
...
```
文書内の一貫性チェック:
- フェーズ構成: セクション6のフェーズリスト == セクション7のフェーズリスト
  → 不一致検出時: [整合性エラー: フェーズ構成が一致していません] を挿入
...
機能要件 → REQ-XXX where type == "functional"
  - タイトル、概要、詳細、優先度を表形式で出力
  - ※ IDは成果物には記載しない（内部参照のみ使用）
...
| **追跡可能性** | 要件の背景・理由が記載されているか | 背景不明の要件 |
...
**重要**: 成果物には引用元のID（REQ-XXX、DEC-XXX、RSK-XXX等）を記載しない。

- IDはproject_state内での管理・追跡用であり、顧客向け成果物には不要
- 要件や決定事項の内容（タイトル、概要、詳細）のみを記載
- ただし、追跡可能性のため、メタデータには参照元ファイルを記録する
