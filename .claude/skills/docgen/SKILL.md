...
生成した成果物の完全性を確認する（G-1～G-4）。

**チェック内容:**

1. **G-1: テンプレート必須セクション存在確認**
   - completeness-rules.yamlで定義された必須セクションが全て存在するか
   - 例: 提案書の「エグゼクティブサマリー」「課題認識」等
...
5. **G-5: フェーズ構成の一貫性確認（project_planのみ）**
...
[成果物完全性チェック]
- 対象: outputs/proposal_draft.md
- ルール: completeness-rules.yaml

結果:
- Critical: 1件
  - [CRITICAL] 「6.2 プラットフォーム特徴」セクションが空です

- Warning: 2件
  - [WARNING] 「10. セキュリティ・ガバナンス」に[TBD]マークが残っています
  - [WARNING] 表紙の「提出日」が未記入です

判定:
- Critical → [要入力]マークを挿入、警告を出力
- Warning → 警告を出力
- 生成は継続（後でQuality-Gateで再チェック）
```

**実行例（プロジェクト計画書）:**
```
[成果物完全性チェック]
- 対象: outputs/project_plan_draft.md
- ルール: completeness-rules.yaml

結果:
- Critical: 1件
  - [CRITICAL] フェーズ構成が一致していません
    - セクション6: Phase1, Phase2, Phase3, Phase4
    - セクション7: Phase1, Phase2, Phase3
    - 差分: Phase4がセクション7に欠落

判定:
- Critical → [整合性エラー: セクション6と7のフェーズリストを一致させてください] を挿入
- 生成は継続（修正が必要）
```

**対応:**
- Critical/Warning検出時は、該当箇所に適切なマークを挿入
  - 空セクション、未入力: `[要入力: 項目名]`
  - フェーズ一貫性エラー: `[整合性エラー: セクション6と7のフェーズリストを一致させてください]`
- 警告メッセージをユーザーに表示
- 生成は継続（提出前にQuality-Gateで修正）

**注意:**
- DocGenでは検出と警告のみ実施
- 詳細な品質チェックと修正指示はQuality-Gateで実施
