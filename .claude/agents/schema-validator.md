...
以下のバリデーションを実行（役割分担マトリクスのA～E）：

| カテゴリ | 内容 |
|---------|------|
| **A. データフォーマット** | required, pattern, enum, 日付フォーマット、ID採番ルール |
| **B. 参照整合性** | reference_integrity（参照先ID存在確認、循環参照検出） |
| **C. 日付整合性** | date_consistency（due >= created_at、依存関係の日付整合性） |
| **D. ビジネスルール** | business_rules（Blocked時のblocker必須、Resolved時のresolution必須、Realized時のissues登録、スコア自動計算） |
| **E. 状態遷移** | state_transitions（許可された状態遷移のみ、final状態からの遷移禁止） |
