...
inputs/
  ...
  hearing_digests/   # 構造化抽出物（追跡と差分反映用）
  references/        # 補足資料
...
### 運用フロー一括実行
```
「ヒアリング後の全体更新を実行」  # Intake → WBS → State-Reviewer → QualityGate
「週次サイクルを回して」          # WBS → State-Reviewer → WeeklyReport → QualityGate
```

## 品質保証の階層

品質担保は3層構造で実現。各層で責務を明確に分離。

```
┌─────────────────────────────────────────────────────────────┐
│ 第1層: データ定義層（スキーマがマスター）                    │
│  └─ project_state/schemas/*.schema.yaml                     │
├─────────────────────────────────────────────────────────────┤
│ 第2層: 予防的バリデーション（更新時）                        │
│  ├─ Hook層（validate_schema.py）: 基本フォーマット検証       │
│  │   - 必須フィールド、パターン、enum、日付形式、ID重複      │
│  └─ schema-validator Agent: 詳細バリデーション              │
│      - 参照整合性、日付整合性、ビジネスルール、状態遷移      │
├─────────────────────────────────────────────────────────────┤
│ 第3層: 事後確認チェック（レビュー時）                        │
│  ├─ Quality-Gate Skill: 成果物の完全性・整合性              │
│  │   - 必須セクション、TODO検出、ドキュメント間整合性        │
│  └─ State-Reviewer Agent: project_state横断整合性           │
│      - リスク↔課題整合性、長期未更新検出、ID欠番            │
└─────────────────────────────────────────────────────────────┘
```

### Hook層 vs Agent層の責務分離

| チェック種別 | Hook層 | schema-validator Agent |
|------------|--------|----------------------|
| A. 必須フィールド、パターン、enum | **実施** | - |
| B. 参照整合性（他ファイル参照） | - | **実施** |
| C. 日付整合性（依存関係含む） | - | **実施** |
| D. ビジネスルール | - | **実施** |
| E. 状態遷移 | - | **実施** |

### DocGen vs Quality-Gate の責務分離

| チェック種別 | DocGen | Quality-Gate |
|------------|--------|--------------|
| G-1: 必須セクション存在 | - | **実施** |
| G-2: 空セクション検出 | マーク挿入 | **実施** |
| G-3: TODO/TBDマーク | - | **実施** |
| G-4: 必須フィールド | - | **実施** |
| G-5: フェーズ一貫性 | **実施** | - |
| レポート生成 | なし | **生成** |
